<%@ Page Title="" Language="C#" MasterPageFile="~/Master/Content.Master" AutoEventWireup="true" CodeBehind="printoffsetchargefeeall.aspx.cs" Inherits="Web.PrintPage.printoffsetchargefeeall" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script src="../js/vue.js"></script>
    <script>
        var tdWidth, tdHeight;
        $(function () {
            tdWidth = $("#<%=this.tdWidth.ClientID%>");
            tdHeight = $("#<%=this.tdHeight.ClientID%>");
        })
    </script>
    <script src="../js/Page/Print/printoffsetchargefeeall.js?v=<%=base.getToken() %>"></script>
    <script src="../js/Lodop/LodopFuncs.js?t=<%=base.getToken() %>"></script>
    <script type="text/javascript">
        var pageWidth, pageHeight, PrintIDList = [];
        function printpage() {
            saveprocess(false);
        };
        function startPrint() {
            ...
            $.each(content.list, function (index, item) {
                PrintIDList.push(item.PrintID);
            })
            var url = "printoffsetchargefeeview.aspx?PageWidth=" + pageWidth + "&PageHeight=" + pageHeight;
            iframe.src = url;
        ...
        <div class="table_container">
            ...
            <div class="wrap" id="print_wrap">
                <div class="content" id="print_content">
                    <table class="sfxm" v-for="item in list">
                        <thead>
                            <tr>
                                <th colspan="7" class="skdh">冲抵单号:                
                                <input type="text" v-model="item.PrintNumber" />
                                </th>
                            </tr>
                            <tr>
                                <th colspan="7" class="systitle">
                                    <label>{{item.FirstTitle}}</label>
                                </th>
                            </tr>
                            <tr>
                                <th colspan="7" class="sksj">冲抵单据
                                </th>
                            </tr>
                            <tr>
                                <th colspan="7" style="width: 100%; padding-bottom: 5px;">
                                    <div class="fjdz finalfont">
                                        资源地址:                
                                    <input v-model="item.RoomFullName" type="text" />
                                    </div>
                                    <div class="khxm finalfont">
                                        客户名称:                
                                    <input v-model="item.RoomOwnerName" type="text" />
                                    </div>
                        ...
                        <tbody>
                            <tr class="haveborder" v-for="item2 in item.list">
                                <td>{{item2.ChargeName}}
                                </td>
                                <td>{{item2.StartTime}}
                                </td>
                                ...
                                <td>{{item2.FinalStartPoint}}/{{item2.FinalEndPoint}}
                                </td>
                                <td>{{item2.Remark}}
                                </td>
                            </tr>
                            <tr class="haveborder">
                                <td class="lefttd" colspan="3">应收合计: ￥{{item.TotalCost}}
                                </td>
                                <td class="lefttd" colspan="4">实收合计: ￥{{item.money}}</td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
                                    ...
                                    <table class="skxx">
                                        <tr>
                                            ...
                                            <td class="skr" style="width: 60px;">
                                                <input v-model="item.ChargeMan" type="text" /></td>
                                            <td style="width: 65px;">冲抵时间: </td>
                                            <td class="skrq" style="width: 200px">
                                                <input style="width: 150px;" v-bind:id="'chargetime_'+item.RoomID" class="easyui_time" v-model="item.ChargeTime" type="text" /></td>
                                            <td style="width: 55px;">冲销项目: </td>
                                            <td class="skrq" style="width: 150px">
                                                <select v-model="item.ChargeForSummary" style="width: 100px;" class="easyui_combobox">
                                                    <option value="预收款">预收款</option>
                                                </select></td>
                                            <td style="width: 55px;">冲销方式: </td>
                                            <td class="skrq" style="width: 150px">
                                                <select style="width: 100px;" v-model="item.ChargeMethods" class="easyui_combobox">
                            ...
                            <tr>
                                <th class="shuoming" colspan="7">说明:<input v-model="item.Remark" type="text" />
                                </th>
                            </tr>
                        </tfoot>
                    </table>
