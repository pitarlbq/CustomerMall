...
function loadtt() {
    $('#tt_table').datagrid({
        ...
        { field: 'IsCustomerAPPSendDesc', title: '业主端显示', width: 100 },
        { field: 'IsUserAPPSendDesc', title: '员工端显示', width: 100 },
        { field: 'IsWechatSendDesc', title: '微信端显示', width: 100 },
...
function addRow() {
    var iframe = "../Wechat/EditMsg.aspx";
    do_open_dialog('新增通知', iframe);
}
function editRow() {
    var rows = $("#tt_table").datagrid("getSelections");
    if (rows.length == 0) {
        show_message('请至少选择一个通知进行此操作', 'warning');
        return;
    }
    if (rows.length > 1) {
...
function EditDataByRow(row) {
    var iframe = "../Wechat/EditMsg.aspx?ID=" + row.ID;
    do_open_dialog('修改通知', iframe);
}
function removeRows() {
    var rows = $('#tt_table').datagrid('getSelections');
    if (rows.length == 0) {
        show_message('请选择需要删除的数据', 'warning');
        return;
    }
    var IDList = [];
    $.each(rows, function () {
...
function sendNotify() {
    var rows = $('#tt_table').datagrid('getSelections');
    if (rows.length == 0) {
        show_message('请选择需要发送的通知', 'warning');
        return;
    }
    var IDList = [];
    $.each(rows, function () {
