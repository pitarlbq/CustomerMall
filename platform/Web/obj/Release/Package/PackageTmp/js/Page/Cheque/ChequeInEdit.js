...
function loadattachs() {
    var options = { visit: 'loadinsummaryfiles', ID: InSummaryID };
    $.ajax({
        type: 'POST',
        dataType: 'json',
...
function deleteAttach(AttachID) {
    top.$.messager.confirm("提示", "是否删除选已上传的文件?", function (r) {
        if (r) {
            var options = { visit: 'removeinsummaryfile', ID: AttachID };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '../Handler/ChequeHandler.ashx',
...
function getlatestsummary() {
    var options = { visit: 'getlatestcheckinsummary' };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '../Handler/ChequeHandler.ashx',
...
function addProductList() {
    var options = { visit: 'addtempproductlist', list: JSON.stringify(productlist), guid: guid };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '../Handler/ChequeHandler.ashx',
...
function addRow() {
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../Cheque/AddInProduct.aspx?guid=" + guid + "&InSummaryID=" + InSummaryID + "' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
...
function editRow() {
    ...
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../Cheque/AddInProduct.aspx?ID=" + row.ID + "' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
    $("<div id='winaddproduct'></div>").appendTo("body").window({
        title: '修改商品',
        width: ($(window).width() - 200),
        height: $(window).height(),
...
function removeRow() {
    ...
    top.$.messager.confirm('info', '确认删除?', function (r) {
        if (r) {
            var options = { visit: 'removechequeindetail', IDList: JSON.stringify(idlist) };
...
function saveRows() {
    ...
    $('#ff').form('submit', {
        ...
        onSubmit: function (param) {
            param.visit = 'savechequein';
            param.InSummaryID = InSummaryID;
