...
function loadtt() {
    $('#tt_table').datagrid({
        ...
        { field: 'PropertyCategoryName', title: '分类名称', width: 100 },
        { field: 'AddTime', formatter: formatTime, title: '添加时间', width: 100 },
...
function SearchTT() {
    ...
    $("#tt_table").datagrid("load", {
        "visit": "loadpropertycategorygrid",
        "keywords": keywords
    });
}
...
function addRow() {
    var iframe = "../CangKu/EditPropertyCategory.aspx";
    do_open_dialog('新增资产类别', iframe);
}
function editRow() {
...
function editByRow(ID) {
    var iframe = "../CangKu/EditPropertyCategory.aspx?ID=" + ID;
    do_open_dialog('修改资产类别', iframe);
}
function removeRow() {
    var rows = $("#tt_table").datagrid("getSelections");
    if (rows.length == 0) {
    ...
    top.$.messager.confirm("提示", "是否删除选中的供应商", function (r) {
        if (r) {
            var options = { visit: 'removepropertycategory', IDList: JSON.stringify(IDList) };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '../Handler/CKHandler.ashx',
