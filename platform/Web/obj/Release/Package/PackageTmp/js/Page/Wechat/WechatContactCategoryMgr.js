...
function SearchTT() {
    ...
    $("#tt_table").datagrid("load", {
        "visit": "loadwechatcontactcategorygrid"
    });
}
...
function savetype() {
    ...
    var options = { visit: 'savewechatcontactcategory', id: id, name: name, sortorder: sortorder };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '../Handler/WechatHandler.ashx',
...
function deletetype() {
    ...
    top.$.messager.confirm("提示", "确认删除?", function (r) {
        if (r) {
            var options = { visit: 'deletewechatcontactcategory', ids: JSON.stringify(IDList) };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '../Handler/WechatHandler.ashx',
