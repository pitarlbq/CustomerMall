...
function getFields() {
    MaskUtil.mask();
    var options = { visit: 'getchatkeywords' };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        ...
        success: function (data) {
            ...
            $('#keywords_field').html('<div class="title">屏蔽关键字</div>');
            if (data.keywords_list.length == 0) {
                addkeywords();
                return;
            }
            $.each(data.keywords_list, function (index, item) {
                addColumnField('keywords', keywords_count, item.ID, item.Name);
                keywords_count = item.ID;
            })
        }
    });
}
function addColumnField(name, count, ColumnID, ColumnName) {
    ...
    if (CanRemove == 1) {
        html += "<a href=\"javascript:void(0)\" onclick=\"removekeywords('" + inputid + "','" + name + "')\" class=\"easyui-linkbutton btnlinkbar\" data-options=\"plain:true,iconCls:'icon-remove'\">删除</a>"
    }
    html += '</div>';
    $(html).appendTo('#' + name + '_field');
    $.parser.parse("#" + divid);
}
function removekeywords(id, name) {
    id = id.trim();
    ...
    if (data_id > 0) {
        top.$.messager.confirm('提示', '确认删除', function (r) {
            if (r) {
                ...
                var options = { visit: 'removechatkeywords', ID: data_id };
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: '../Handler/MallHandler.ashx',
...
function savekeywords(name) {
    if ($('input.' + name + 'column').length == 0) {
        show_message('请添加关键字', 'warning');
        return;
    }
    var IDList = [];
    var List = [];
    ...
    if (List.length == 0) {
        show_message('参数名称不能为空', 'warning');
        return;
    }
    MaskUtil.mask('body', '提交中');
    var options = { visit: 'savechatkeywords', list: JSON.stringify(List) };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '../Handler/MallHandler.ashx',
...
function addkeywords() {
    keywords_count++;
    addColumnField('keywords', keywords_count, 0, "");
}
