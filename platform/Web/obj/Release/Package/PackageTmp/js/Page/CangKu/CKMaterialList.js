...
function loadTT() {
    ...
    $('#tt_table').datagrid({
        ...
        { field: 'ChukuStatus', title: '是否出库', width: 100 },
...
function onLoadSuccess(data) {
    MergeCells("tt_table", "ck,ServiceNumber,AddTime,PayStatusDesc,ChukuStatus,HandelFee,TotalFee,BalanceStatusDesc");
}
...
function doTransferOut() {
    ...
    if (rows[0].CKProductOutSumaryID > 0) {
        show_message("物料清单已出库", "warning");
        return;
    }
    var iframe = "../CangKu/EditCkOut.aspx?ServiceID=" + rows[0].ServiceID;
    do_open_dialog('转为出库单', iframe);
}
