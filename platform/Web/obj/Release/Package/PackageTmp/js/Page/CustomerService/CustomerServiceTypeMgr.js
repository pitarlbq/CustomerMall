...
function savetype() {
    ...
    var options = { visit: 'saveservicetype', id: id, name: name, sortorder: sortorder };
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '../Handler/ServiceHandler.ashx',
...
function edittype() {
    ...
    $("#tdID").val(rows[0].ID);
    $("#tdName").textbox("setValue", rows[0].ServiceTypeName);
...
function deletetype() {
    ...
    top.$.messager.confirm("提示", "确认删除?", function (r) {
        if (r) {
            var options = { visit: 'deleteservicetype', ids: JSON.stringify(IDList) };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '../Handler/ServiceHandler.ashx',
