...
function loadtt() {
    $('#tt_table').datagrid({
        ...
        { field: 'FilePath', formatter: formatFilePath, title: '文件地址', width: 100 },
        { field: 'SqlPath', formatter: formatFilePath, title: '脚本地址', width: 100 },
...
function addRow() {
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../SysSeting/EditVersion.aspx' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
...
function EditByRow(row) {
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../SysSeting/EditVersion.aspx?VersionID=" + row.ID + "' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
    parent.$("<div id='winadd'></div>").appendTo("body").window({
        title: '修改版本',
        width: $(parent.window).width() - 400,
        height: $(parent.window).height(),
