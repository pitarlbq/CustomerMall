...
function onCheck(event, treeId, treeNode) {
    if (treeType == 6) {
        if (!treeNode.IsLocked) {
            try {
                var iframeTarget = $('iframe#top_service_frame').contents().find('iframe#dialog_frame2');
                iframeTarget[0].contentWindow.pageLoad(treeNode.ID);
            } catch (e) {
                //alert(e)
            }
            try {
                var iframeTarget = $('iframe#top_payservice_frame').contents().find('iframe#dialog_frame1');
                iframeTarget[0].contentWindow.pageLoad(treeNode.ID);
            } catch (e) {
            }
            try {
                var iframeTarget = $('iframe#top_printformat_frame');
                iframeTarget[0].contentWindow.pageLoad(treeNode.ID);
            } catch (e) {
            }
            try {
                var iframeTarget = $('iframe#top_paramset_frame');
                iframeTarget[0].contentWindow.pageLoad(treeNode.ID);
            } catch (e) {
            }
        }
...
function addDiyDom(treeId, treeNode) {
    ...
    if (treeType == 7) {
        ...
        if (!treeNode.isParent) {
            var editStr = '';
            if (treeNode.IsLocked) {
                editStr = "<input type='radio' disabled='disabled' name='checkbox_0' class='checkboxBtn' id='" + treeNode.id + "' onfocus='this.blur();'></input>";
            }
            else {
                editStr = "<input type='radio' onclick='clickRadioBtn()' name='checkbox_0' class='checkboxBtn' id='" + treeNode.id + "' onfocus='this.blur();'></input>";
            }
            aObj.before(editStr);
            var btn = $("#" + treeNode.id);
            if (btn) {
...
function clickRadioBtn() {
    try {
        var iframeTarget = $('iframe#top_chargefee_frame');
        iframeTarget[0].contentWindow.clickRadioBtn();
    } catch (e) {
        alert(e)
    }
}
