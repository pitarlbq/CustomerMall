...
function yyui_menu(ulclass) {
    $(document).ready(function () {
        $(ulclass + ' li').hover(function () {
            ...
            if (ulElem == null || ulElem.offset() == null) {
                return;
            }
            ulElem.show(); //mouseover
            var top = $(this).offset().top;
            var bottom = document.documentElement.clientHeight - top;
            var totalLiCount = ulElem.children("li").length;
            if (bottom >= totalLiCount * 50) {
                ulElem.css('top', '0px');
            } else if (top >= totalLiCount * 50) {
                ulElem.css('top', -(totalLiCount - 1) * 50 + 'px');
            } else {
                ulElem.css('top', -top + 'px');
            }
            //alert(ulElem.children("li").length);
            //alert(ulElem.offset().top);
        }, function () {
