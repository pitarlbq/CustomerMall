...
function loadChargeFees(ID, RoomID) {
    var options = { visit: "loadsetfeelist", ID: ID, CompanyID: CompanyID, RoomID: RoomID, FeeType: 1 };
    $.ajax({
        type: 'POST',
        dataType: 'json',
...
function initialyt(obj) {
    ...
    html += '<input type="button" class="removebutton buttonmin" onclick="removeFee(' + obj.summary.ID + ')" value=""/>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
    html += '</td>';
        ...
        success: function (data) {
            ...
            if (data.status) {
                if (data.errornames.length == 0) {
                    ...
                    show_message("启用成功,以下房间已启用该收费标准:" + JSON.stringify(data.errornames), "info");
                }
            }
            else {
...
function viewByRoom() {
    ...
    var iframeurl = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../SetupFee/ViewRoomFeeList.aspx?RoomIDs=" + JSON.stringify(roomidarry) + "&ProjectID=" + projectid + "' style='width:100%;height:99%'></iframe>";
...
function addChargeSummary() {
    var iframeurl = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../SetupFee/AddNormalSummaryFee.aspx?FeeType=1' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
    parent.$("<div id='winadd'></div>").appendTo("body").window({
        title: '新增常用费用',
        width: $(window).width() - 100,
        height: $(window).height(),
