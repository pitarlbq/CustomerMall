...
function gettablecolumn() {
    var options = { visit: 'loadtablecolumn', pagecode: 'chequeoutmgr' };
    $.ajax({
        type: 'POST',
        dataType: 'json',
...
function addRow() {
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../Cheque/EditChequeOut.aspx' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
...
function EditDataByRow(row) {
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../Cheque/EditChequeOut.aspx?ID=" + row.ID + "' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
    $("<div id='winadd'></div>").appendTo("body").window({
        title: '销项修改',
        width: ($(window).width() - 200),
        height: $(window).height(),
...
function removeRows() {
    ...
    if (rows.length == 0) {
        show_message("请至少选择一个销项进行此操作", "info");
        return;
    }
    var IDList = [];
    ...
    top.$.messager.confirm("提示", "确认删除选中的销项?", function (r) {
        if (r) {
            var options = { visit: 'removechequeout', IDList: JSON.stringify(IDList) };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '../Handler/ChequeHandler.ashx',
...
function doSign() {
    ...
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../Cheque/ChequeOutSign.aspx?ID=" + row.ID + "' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
    $("<div id='winadd'></div>").appendTo("body").window({
        title: '签收',
        width: ($(window).width() - 200),
        height: $(window).height(),
...
function doApprove() {
    var row = $("#tt_table").datagrid("getSelected");
    if (row == null) {
        show_message("请选择一个销项进行此操作", "info");
        return;
    }
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../Cheque/ChequeOutApprove.aspx?ID=" + row.ID + "' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
    $("<div id='winadd'></div>").appendTo("body").window({
        title: '审核',
        width: ($(window).width() - 200),
        height: $(window).height(),
...
function doImport() {
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../Cheque/ImportOutSummary.aspx' style='width:100%;height:" + ($(window).height() - 190) + "px;'></iframe>";
    $("<div id='winadd'></div>").appendTo("body").window({
        title: '导入销项',
...
function openTableSetUp() {
    var iframe = "<iframe id='myFrame' scrolling='auto' frameborder='0' src='../SysSeting/TableSetUp.aspx?PageCode=chequeoutmgr' style='width:100%;height:" + ($(window).height() - 40) + "px'></iframe>";
