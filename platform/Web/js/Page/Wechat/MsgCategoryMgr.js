...
function SearchTT() {
    ...
    $("#tt_table").datagrid("load", {
        "visit": "getmsgcategorygrid",
        "keywords": keywords,
    });
}
function addRow() {
    var iframe = "../Wechat/EditMsgCategory.aspx";
    do_open_dialog('新增新闻分类', iframe);
}
function editRow() {
...
function EditDataByRow(row) {
    var iframe = "../Wechat/EditMsgCategory.aspx?ID=" + row.ID;
...
function removeRows() {
    ...
    top.$.messager.confirm('提示', '确认要删除选中的数据?', function (r) {
        if (r) {
            ...
            var options = { visit: 'deletemsgcategory', IDList: JSON.stringify(IDList) };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '../Handler/WechatHandler.ashx',
