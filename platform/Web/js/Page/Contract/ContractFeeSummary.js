var tt_CanLoad = false, OrderType = 'Contract';
...
function loadParams() {
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '../Handler/ContractHandler.ashx?visit=getcontractfeesummarygrid',
...
function loadTT() {
    ...
    $('#tt_table').datagrid({
        ...
        { field: 'FormatCalcualteTotalCost', title: '合同应收', width: 80 },
        { field: 'FormatCalcualtePayCost', title: '已收金额', width: 80 },
        { field: 'FormatCalcualteDiscount', title: '减免金额', width: 80 },
        { field: 'FormatCalcualteRestCost', title: '未收金额', width: 80 },
        ]],
...
function onLoadSuccess(data) {
    OrderType = getRadioValue();
    if (OrderType == 'Project') {
        MergeCells("tt_table", "FullName,Name");
    } else {
        MergeCells("tt_table", "FullName,Name,ContractNo,ContractName,RentName");
    }
}
var checked_id = 0;
var unchecked_id = 0;
...
function get_options() {
    ...
    if (StartTime != '' && EndTime != '') {
        if (stringToDate(StartTime).valueOf() > stringToDate(EndTime).valueOf()) {
            show_message("收费开始日期不能收费结束日期", "info");
            return null;
        }
    }
    var Keywords = tdKeywords.searchbox('getValue');
    var OrderType = getRadioValue();
    var options = {
        "visit": "loadcontractchargelist",
        "RoomIDs": JSON.stringify(roomids),
        "ProjectIDs": JSON.stringify(projectids),
        "IncludeFooter": true,
        "ReadyStartTime": StartTime,
        "ReadyEndTime": EndTime,
        "Keywords": Keywords,
        "ContractStatus": "tongguo",
        "FeeStatus": tdFeeStatus.combobox('getValue'),
        "ChargeSummarys": hdChargeSummary.val(),
        "OrderType": OrderType
    };
    return options;
}
