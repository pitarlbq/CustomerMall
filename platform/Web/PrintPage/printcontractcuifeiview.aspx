<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="printcontractcuifeiview.aspx.cs" Inherits="Web.PrintPage.printcontractcuifeiview" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function GenerateTable() {
            ...
            var options = { visit: "loadprintcontractcuifei", IDs: ids };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                async: false,
                ...
                success: function (data) {
                    ...
                    $.each(data.list, function (newindex, dataitem) {
                        ...
                        var pagetotalcount = parseInt(dataitem.contractfeelist.length / totallines);
                        if (dataitem.contractfeelist.length % totallines > 0) {
                            pagetotalcount = pagetotalcount + 1;
                        }
                        pagecount = 0;
                        pagecost = 0;
                        $.each(dataitem.contractfeelist, function (index, item) {
                            if (index % totallines == 0) {
                                ...
                                $html += '合同编号: ' + dataitem.FullName;
                                $html += '</th>';
                                $html += '<th colspan="4" class="khxm finalfont">';
                                $html += '合同名称: ' + dataitem.OwnerName;
                                $html += '</th>';
                                $html += '</tr>';
                            ...
                            $html += '<td>' + (Number(item.CalcualteTotalCost) < 0 ? 0.00 : toThousands(item.CalcualteTotalCost)) + '</td>';
                            $html += '<td>' + (Number(item.CalcualteRestCost) < 0 ? 0.00 : toThousands(item.CalcualteRestCost)) + '</td>';
                            $html += '<td>' + formatTime(item.CalculateStartTime) + '</td>';
                            $html += '<td>' + formatTime(item.CalculateEndTime) + '</td>';
                            $html += '<td>' + item.PrintRemark + '</td>';
                            $html += '</tr>';
                            pagecost += (parseFloat(item.CalcualteTotalCost) < 0 ? 0 : parseFloat(item.CalcualteTotalCost));
                            if ((index + 1) % totallines == 0 || index == (dataitem.contractfeelist.length - 1)) {
