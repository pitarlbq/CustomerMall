<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="printchargepayserviceview.aspx.cs" Inherits="Web.PrintPage.printchargepayserviceview" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function GenerateTable() {
            ...
            var options = { visit: "loadprintchargefee", PrintID: "<%=Request.QueryString["PrintID"]%>", type: "payservice" };
            $.ajax({
                type: 'POST',
                dataType: 'json',
                async: false,
                    ...
                    $.each(message.list, function (index2, data) {
                        ...
                        $.each(finallist, function (index, item) {
                            if (index % totallines == 0) {
                                ...
                                $html += '<th style="width: 20%">应付年月';
                                $html += '</th>';
                                $html += '<th style="width: 20%">付款科目';
                                $html += '</th>';
                                $html += '<th style="width: 20%">实付金额';
                                $html += '</th>';
                                $html += '<th style="width: 40%">应付说明';
                                $html += '</th>';
                                $html += '</tr>';
                            }
                            $html += '<tr class="haveborder">';
                            ...
                            if ((index + 1) % totallines == 0 || index == (data.list.length - 1)) {
                                ...
                                $html += '</tr>';
                                $html += '<tr>';
                                $html += '<th class="shuoming" colspan="' + totalcolumns + '">';
                                $html += '<table class="skxx">';
                                $html += '<tr>';
                                $html += '<td style="width: 60%">备注: ';
                                $html += data.printhistory.Remark + '</td>';
                                $html += '<td style="width: 40%;">签名: ';
                                $html += '</td>';
                                $html += '</tr>';
                                $html += '</table>';
                                $html += '</th>';
