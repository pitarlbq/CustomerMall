<%@ Page Title="" Language="C#" MasterPageFile="~/Master/Content.Master" AutoEventWireup="true" CodeBehind="NewAddContract.aspx.cs" Inherits="Web.Contract.NewAddContract" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        var guid = '', op = '', ContractID = 0, TopContractID = 0, canAdd = 0, canEdit = 0, canSaveLog = 0, canPrint = 0, canRent = 0, canView = 0, canNewRent = 0, tabIndex = 0, canChangeRent = 0;
        $(function () {
            guid = "<%=this.guid%>";
            op = "<%=this.op%>";
            ...
            canChangeRent = "<%=this.canChangeRent?1:0%>";
            $('#ContractTab').tabs({
        ...
        function getRooms() {
            try {
                var roomRows = document.getElementById('ContractBasic').contentWindow.$("#tt_room").datagrid('getRows');
                return roomRows;
            } catch (e) {
        ...
        function hideCustomerTab() {
            $('#ContractTab').tabs('getTab', "多方收款").panel('options').tab.hide();
            $("#tabCustomer").hide();
        }
        function showCustomerTab() {
            $('#ContractTab').tabs('getTab', "多方收款").panel('options').tab.show();
            $("#tabCustomer").show();
        }
    </script>
    <style>
        ...
        <div data-options="region:'center',border:false">
            <div class="easyui-tabs" id="ContractTab" data-options="fit:true,tabPosition:'left',headerWidth:100,tabHeight:35" style="width: 100%; height: 99%; border: none;">
                <div title="基本信息" style="padding-top: 5px; background-color: #f0f0f0;">
                    <input type="hidden" value="NewAddContract_Basic.aspx?ID=<%=this.FinalContractID%>&op=<%=this.op%>" />
                    <iframe src="" id="ContractBasic" style="width: 100%; height: 99%; border: 0"></iframe>
                </div>
                <div title="多方收款" style="padding: 0;" id="tabCustomer">
                ...
                <%if (this.ContractID > 0)
                    {%>
                <div title="合同应收" style="padding: 0px">
                    <input type="hidden" value="ContractCharge.aspx?ID=<%=this.ContractID %>" />
                    <iframe src="" style="width: 100%; height: 99%; border: 0"></iframe>
                </div>
                <div title="已收信息" style="padding: 0px">
                    <input type="hidden" value="ContractChargeHistory.aspx?ID=<%=this.ContractID %>" />
                    <iframe src="" style="width: 100%; height: 99%; border: 0"></iframe>
                </div>
                <div title="变更信息" style="padding: 0px">
                    <input type="hidden" value="EditContractHistory.aspx?ID=<%=this.ContractID %>" />
                    <iframe src="" style="width: 100%; height: 99%; border: 0"></iframe>
                </div>
                <%} %>
            </div>
