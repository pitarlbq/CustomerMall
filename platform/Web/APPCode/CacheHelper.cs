    ...
    public class CacheHelper
    {
        ...
        public static Company GetCompany(HttpContext context, bool readCache = true)
        {
            ...
            else
            {
                ...
                bool result = EncryptHelper.GetCompanyInfo(BaseURL, WebUtil.GetFromCompanyID(context), out errormsg, out company, false);
        ...
        public static Project SaveMyWechatCurrentProject(string OpenID, int ProjectID)
        {
            var project = Wechat_User.SaveWechat_UserCurrentProjectID(OpenID, ProjectID);
            var cache = HttpRuntime.Cache;
            string self_wechat_currentproject = wechat_currentproject + "_" + OpenID;
            cache.Remove(self_wechat_currentproject);
            return project;
        }
        public static Project GetMyWechatCurrentProject(string OpenID, bool UseCache = true)
        {
            var cache = HttpRuntime.Cache;
            string self_wechat_currentproject = wechat_currentproject + "_" + OpenID;
            if (cache.Get(self_wechat_currentproject) != null)
            {
                if (UseCache)
                {
                    return cache.Get(self_wechat_currentproject) as Project;
                }
                cache.Remove(self_wechat_currentproject);
            }
            var current_project = Wechat_User.GetWechat_UserCurrentProject(OpenID);
            if (current_project != null)
            {
                cache.Insert(self_wechat_currentproject, current_project);
            }
