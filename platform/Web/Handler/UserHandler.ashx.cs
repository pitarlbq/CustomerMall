...
namespace Web.Handler
{
    ...
    public class UserHandler : IHttpHandler, IRequiresSessionState
    {
            ...
            try
            {
                switch (visit)
                {
                    ...
                    case "savesqlfile":
                        savesqlfile(context);
                        break;
                    case "savebusinessuserinfo":
                        savebusinessuserinfo(context);
                        break;
        ...
        private void savesqlfile(HttpContext context)
        {
            ...
            string errorcompanys = string.Empty;
            Encript.SqlLite.DeleteCompany();
            foreach (var company in companys)
            {
                string errormsg = string.Empty;
                if (!Web.APPCode.SqlLiteHelper.SaveCompany(company, out errormsg))
                {
                    errorcompanys += company.CompanyName;
                }
                string errormsg2 = string.Empty;
                if (!Web.APPCode.SqlLiteHelper.SaveCompanyModule(company, out errormsg2))
                {
                    errorcompanys += company.CompanyName;
                }
            }
            if (!string.IsNullOrEmpty(errorcompanys))
            {
                WebUtil.WriteJson(context, new { status = false, error = errorcompanys });
                return;
            }
            WebUtil.WriteJson(context, new { status = true });
        }
        ...
        private void RemoveUser(HttpContext context)
        {
            ...
            if (IDList.Count > 0)
            {
                ...
                if (app_users.Length > 0)
                {
                    ...
                    delete_success = EncryptHelper.RemoveAPPUser(company, UserIDList, out errormsg);
                }
                if (!delete_success)
                {
                ...
                #region 删除账号日志
                APPCode.CommHelper.SaveOperationLog("删除账号" + UserNames, Utility.EnumModel.OperationModule.RemoveUser.ToString(), "删除账号", IDListArry, "User");
                #endregion
            }
            WebUtil.WriteJson(context, new { status = true });
        }
