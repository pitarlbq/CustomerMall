...
{
    public partial class ChequeInMgr : BasePage
    {
        ...
        private void SetDrValue(DataRow dr, string Title, object Value)
        {
            ...
            {
                titleList = Foresight.DataAccess.TableColumn.GetTableColumnByPageCode("chequeinmgr", true).Where(p => !p.ColumnName.Equals("选择按钮")).ToArray();
            }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var list = Foresight.DataAccess.ViewChequeInSummary.GetViewChequeInSummaryListByKeywords(keywords, SellerList, ProductList, DepartmentList, ProjectList, StartTime, EndTime, ChequeCode);
                if (list.Length > 0)
                {
                    titleList = Foresight.DataAccess.TableColumn.GetTableColumnByPageCode("chequeinmgr", true).Where(p => !p.ColumnName.Equals("选择按钮")).ToArray();
                    ...
                    string FileName = "进项-" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
