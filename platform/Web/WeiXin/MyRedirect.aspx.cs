...
{
    public partial class MyRedirect : WechatBasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    activitypath = "/html/newweixin/myinfo/index.html";
                }
                file = Server.MapPath("~" + activitypath);
                if (System.IO.File.Exists(file))
                {
                    this.Response.Redirect(WeixinHelper.ResolveClientUrl(activitypath + "?t=" + DateTime.Now.Ticks + "&index=2"), true);
                    return;
                }
                else
                {
