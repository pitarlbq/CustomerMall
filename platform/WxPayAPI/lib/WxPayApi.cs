...
namespace WxPayAPI
{
    public class WxPayApi
    {
        ...
        */
        ...
        public static WxPayData OrderQueryAPP(WxPayData inputObj, int timeOut = 6)
        {
            string url = "https://api.mch.weixin.qq.com/pay/orderquery";
            //检测必填参数
            if (!inputObj.IsSet("out_trade_no") && !inputObj.IsSet("transaction_id"))
            ...
            inputObj.SetValue("appid", PaymentConfig.WeiXinConfig.MobileAppID);//公众账号ID
            inputObj.SetValue("mch_id", PaymentConfig.WeiXinConfig.MobileMCHID);//商户号
            inputObj.SetValue("nonce_str", WxPayApi.GenerateNonceStr());//随机字符串
            inputObj.SetValue("sign", inputObj.MakeAPPSign());//签名
