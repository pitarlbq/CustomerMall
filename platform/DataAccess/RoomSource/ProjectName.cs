...
namespace Foresight.DataAccess
{
    ...
    {
        public static ProjectName GetProjectNameByName(string Name, string PName)
        {
            using (SqlHelper helper = new SqlHelper())
            {
                return GetProjectNameByName(Name, PName, helper);
            }
        }
    ...
    public class ProjectNameDetails : ProjectName
    {
        ...
        public static ProjectNameDetails[] GetProjectNameListWherePIDIsZero(int CompanyID)
        {
            List<SqlParameter> parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("@CompanyID", CompanyID));
            return GetList<ProjectNameDetails>("select *,(select top 1 [OrderBy] from [ProjectYT] where [Name]=[ProjectName].PName and [ID]=[ProjectName].PID) as OrderBy from [ProjectName] where [ID] in (select [ID] from [ProjectNameRelation] where [PID]=0)", parameters).ToArray();
        }
    }
}
