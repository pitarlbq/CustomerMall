...
namespace Foresight.DataAccess
{
    ...
    {
        public static JPushLog[] GetJPushLogListByRelatedID(int RelatedID, int PushType, bool IsPushed)
        {
            List<string> conditions = new List<string>();
            List<SqlParameter> parameters = new List<SqlParameter>();
            if (RelatedID > 0)
            {
                conditions.Add("[RelatedID]=@RelatedID");
                parameters.Add(new SqlParameter("@RelatedID", RelatedID));
            }
            if (PushType > 0)
            {
                conditions.Add("[PushType]=@PushType");
                parameters.Add(new SqlParameter("@PushType", PushType));
            }
            if (IsPushed)
            {
                conditions.Add("[IsPushed]=1");
            }
            if (conditions.Count == 0)
            {
                return new JPushLog[] { };
            }
            return GetList<JPushLog>("select * from [JPushLog] where " + string.Join(" and ", conditions.ToArray()), parameters).ToArray();
        }
        /// <summary>
        /// 
        /// </summary>
