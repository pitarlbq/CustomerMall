...
namespace ExcelProcess
{
    public class ExcelExportHelper
    {
        ...
        private static void SetCellValue(IWorkbook workbook, string ColumnName, ICell cell, string strValue)
        {
            ...
            ICellStyle styleFormat = workbook.CreateCellStyle();
            styleFormat.DataFormat = dataformat.GetFormat("#,##0.00");//分段添加，号
            strValue = string.IsNullOrEmpty(strValue) ? "" : strValue;
            if (strValue.Contains(".") || strValue.Equals("0"))
            {
                doubleValue = 0;
            ...
            if (string.IsNullOrEmpty(strValue) || strValue.Length > 10 || strValue.StartsWith("0") || ColumnName.Contains("ID") || strValue.Contains(","))
            {
                cell.SetCellType(CellType.STRING);
                cell.SetCellValue(strValue);
                return;
            }
            if (Utility.Tools.IsNumeric(strValue))
            {
                doubleValue = 0;
